...
class stilix_list_count:
          value: int
          def list_data(self,data_: any,desired_counts: list):
                    for item in data_:
                              if item in desired_counts:
                                        self.value += 1
                    return self.value
          
          def data_lengths(self,datas: list[any]):
                    for data in datas:
                              return len(data)
          
          def reveal_item_indices(self,data_containing_item: any,items: list):
                    for item in items:
                              return data_containing_item.index(item)
...
class stilix:
          ...
          def insert_style(self,storage: list):
                    value = 1
                    while value < STILIX_LIST_COUNT.list_lengths([storage]):
...
'''
The BRIGHTNESS object can be used to change the brightness of your style.You can either make your style bright or you can make your style dim.
-
STILIX.change_style("style")
-
...
dim->style
-
note:I cant change the brightness of the output so I am writing brightness->output.
-
BRIGHTNESS.bright()
print(STILIX.style)
